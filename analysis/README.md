...
| `knob_tolerant_null.py` | **All-revision generalisation of `v1l_blend_knob_probe.py`'s method, plus a linear-removed floor.** Runs the same local BLEND/DRIVE search (±`--span`, default 0.05) on every capture across all three revisions (use `--exclude "V2-2"` to stick to the 11 original-unit captures — see the top-of-file note on why `V2-2` shouldn't be pooled), and additionally renders once at the exact labelled setting to report `analyze.linear_removed_null` alongside the raw null, so a shallow null can be read as "linear-fixable" vs "at the nonlinear/capture floor" without a second script. Prints a best-null-per-revision summary (2026-07-23: V1E −18.0 dB / V1L −10.8 dB / V2 −16.8 dB knob-tolerant; linear-removed floors 10–20 dB deeper on all three — see the top-level README's Accuracy section). ⚠ **A real bug lived here for a while and is worth remembering the shape of**: the local search picked the offset with the *largest* (least negative) null value as "best" — backwards, since null_depth() is dB re reference RMS and more negative is deeper/better. It went undetected because the printed numbers still *looked* plausible in isolation; the tell was a sanity check that should always hold and didn't — the search space includes the labelled (nominal) setting, so a correctly-computed "best" can never be shallower than nominal, and the buggy version routinely reported one that was. Fixed (`nd < best`, not `nd > best`); re-verify with that same sanity check if this script is ever extended. **2026-07-23 addendum: added the boundary-exclusion guard from `v1l_blend_knob_probe.py`** (an optimum sitting on the swept edge is a non-result, not a measurement) — every V1L capture hit the ±0.05 default edge; re-run with a widened span confirmed V1L's headline best null is still −10.8 dB (that capture was already interior), but the V1030/BL1.00 capture's TRUE interior optimum is only −7.14 dB at a rendered blend of 0.50 (shift −0.50) — inconsistent with the other two V1L captures' small, near-zero shifts, so per the existing decision rule this is a one-off capture/knob discrepancy (see CLAUDE.md's "V1L blend/wet-level discrepancy" open item), not a systematic taper defect worth fixing. | `--os`, `--span`, `--steps`, `--seg`, `--filter`, `--exclude`, `--csv` |
...
| `v1l_m_scan.py` | **V1L independent zener-m fit attempt (2026-07-23) — REFUTED, do not re-run expecting different.** n=1 (V1030 is V1L's only full-wet file); H2-primary/H4-secondary scoring with an odd-harmonic premise control; `--blend-override 0.50` tests the corrected-blend hypothesis (both run: fit fails under either — best H2 residual ~8.4 dB, H2/H4 disagree on the optimum). Side finding: odd-control errors halve under blend=0.50, corroborating the null-sweep's "V1030's real knob ≈ 0.50" read. | `--os`, `--values`, `--blend-override` |
| `v1l_rail_scan.py` | **V1L stage-A rail-asymmetry scan (2026-07-23) — lever physically DEAD for the LF even-harmonic deficit.** Carries its own L-009 liveness gate (`gapd_flag_check.py` never covered `--rail-vneg/vpos`): flag live, value plumbing proven (opposite extremes differ −21 dB), yet all 7 pairs score bit-identically at 100–400 Hz — the zener clamps at LF before the wiper rails, exactly as ZenerDriveModule.h documents. Plus the class-level kill: pedal H2-vs-level slope RISES (+7.7 dB, 200 Hz) where every static asymmetry's falls ⇒ the deficit needs MEMORY (bias-node droop), not a clip-element parameter. See CLAUDE.md / session log 2026-07-23. | `--os`, `--pairs`, `--blend-override` |
| `v1l_biasdroop_feasibility.py` | ⭐ **PAPER-only bias-node-droop feasibility (2026-07-23, no renders) — verdict MARGINAL, do not build.** Solves the shipped zener law with a level-tracking centre offset δ; scores the E2-free observable (H2-slope differences, 100/200 Hz; 400 Hz excluded per Gap G). Keeps two durable results: (1) window vs input-referred offsets are mathematically IDENTICAL in this feedback topology (substitution + bit-identical numerics) — one offset knob exists, so the test covers the class; (2) unlike every static asymmetry, the level-tracking offset HAS the authority for rising H2 slopes (~0.73 dB rms ceiling, odds clean) — but needs 0.2→1.5 V droop at estimated clip amplitudes (pins the physical ceiling) and its only evidence is one blend-mislabelled capture on a FINAL matrix (guardrail #6 wall). Own static-δ control reproduces the falling-slope refutation, proving the harness discriminates. | — |
| `check_asym_sources.py` | Compares asymmetric rails vs sat-offset as H2 sources | `--os` |
| `cj_scan.py` | Zener junction capacitance fit from DRIVE HF rolloff. `--rev V2` (default) or `--rev V1L`. **2026-07-23: `--rev V1L` result is NOT shippable** — the RMS HF-shape error is nearly flat (7.53–7.65 dB) across 150–470 pF (vs V2's decisive 4.7 dB minimum), and the per-capture HF errors flip sign across the 3 captures (V1030 plugin darker than pedal at 8/12 kHz, V1100 plugin brighter at the same frequencies) — the guardrail #6/L-008 overfitting signature from only 3, mutually-confounded captures, not a real Cj value. V1L keeps its Phase-4 220 pF estimate. | `--rev`, `--values`, `--os`, `--min-blend`, `--max-drive` |
