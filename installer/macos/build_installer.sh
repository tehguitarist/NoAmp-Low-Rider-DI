#!/usr/bin/env bash
# TEMPLATE: replace NoAmpLowRiderDI below with your plugin's product name (must match juce_add_plugin's
# PRODUCT_NAME / the *_artefacts directory name CMake produces). Inert until copied to a repo root.
#
# Builds a macOS .pkg installer for NoAmpLowRiderDI from already-built Release artefacts, with a
...
#   [artefacts-dir]  defaults to build/NoAmpLowRiderDI_artefacts/Release (repo-root relative)
#   [output-dir]     defaults to the current directory
#
# Requires the AU and VST3 targets to already be built (NoAmpLowRiderDI_AU, NoAmpLowRiderDI_VST3).
...
ARTEFACTS="${2:-build/NoAmpLowRiderDI_artefacts/Release}"
OUTDIR="${3:-.}"
...
pkgbuild --root "$AU_ROOT" \
    --identifier com.leighpierce.noamplowriderdi.au \
    --version "$VERSION" \
    --install-location "/Library/Audio/Plug-Ins/Components" \
    "$WORK/au.pkg"

pkgbuild --root "$VST3_ROOT" \
    --identifier com.leighpierce.noamplowriderdi.vst3 \
    --version "$VERSION" \
...
mkdir -p "$OUTDIR"
OUT_PKG="$OUTDIR/NoAmpLowRiderDI-macOS-v${VERSION}-Installer.pkg"
