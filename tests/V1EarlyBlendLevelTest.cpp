...
double clampPot(double r) { return r < kMin ? kMin : r; }
...
cd blendMNA(double f, double blend, double level, double dryV, double wetV)
{
    ...
    auto stampY = [&](int a, int b, cd y, double va, double vb) {
        ...
        if (a < 0 && b >= 0) rhs[b] += y * va; // a is a source node held at va
        if (b < 0 && a >= 0) rhs[a] += y * vb;
    };
    ...
    M[5][8] += 1.0; M[8][3] += 1.0; M[8][5] -= 1.0;
    // IC4B inverting: p=datum, n=6, out=7. Row N+1 = 9.
    M[7][9] += 1.0; M[9][6] += 1.0; // V6 - V(datum) = 0 -> V6 = 0
    ...
    for (int c = 0; c < DIM; ++c)
    {
        ...
        for (int j = 0; j < DIM; ++j) std::swap(M[c][j], M[piv][j]);
        std::swap(rhs[c], rhs[piv]);
        const cd d = M[c][c];
        for (int j = 0; j < DIM; ++j) M[c][j] /= d;
        rhs[c] /= d;
        for (int r = 0; r < DIM; ++r)
        {
            if (r == c) continue;
            const cd fct = M[r][c];
            for (int j = 0; j < DIM; ++j) M[r][j] -= fct * M[c][j];
            rhs[r] -= fct * rhs[c];
        }
    }
    return rhs[7];
...
int main()
{
        ...
        for (double b : { 0.0, 0.25, 0.5, 0.75, 1.0 })
            for (double l : { 0.1, 0.3, 0.5, 0.75, 1.0 })
            {
                ...
                for (int which = 0; which < 2; ++which)
                {
                    ...
                    if (refDb < -70.0) continue;
                    st.reset();
                    if (std::abs(meas - refDb) > std::abs(worst)) { worst = meas - refDb; wb = b; wl = l; ww = which; }
                }
            }
    ...
    {
        st.setBlendLevel(0.0, 1.0); st.reset(); // full dry
        const double wetAtDry = measureDb(st, fs, f, 1);
        st.setBlendLevel(1.0, 1.0); st.reset(); // full wet
        ...
        std::printf("      @1kHz full-dry wet-leak: nodal %.1f / ref %.1f dB; full-wet dry-leak: nodal %.1f / ref %.1f dB\n",
    ...
    {
        st.setBlendLevel(1.0, 1.0); st.reset();
        ...
        for (double l : { 0.1, 0.3, 0.5, 0.75, 1.0 })
        {
            st.setBlendLevel(1.0, l); st.reset();
            const double g = measureDb(st, fs, f, 1);
            if (g < prev - 0.05) mono = false;
            prev = g;
        }
