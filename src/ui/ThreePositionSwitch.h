...
public:
    ...
    // Optional bitmap override (src/ui/PedalAssets.h): three fully-composited images, one per
    // switch position, drawn into the body rect in place of the vector lever below. Unset
    // (invalid) images leave the vector drawing unchanged — stays a generic reusable peripheral.
    void setBodyImages(juce::Image top, juce::Image mid, juce::Image bottom)
    {
        bodyImage[0] = std::move(top);
        bodyImage[1] = std::move(mid);
        bodyImage[2] = std::move(bottom);
        repaint();
    }
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        if (bodyImage[position].isValid())
        {
            g.drawImage(bodyImage[position], juce::Rectangle<float>(bodyX, bodyY, kBodyW, bodyH),
