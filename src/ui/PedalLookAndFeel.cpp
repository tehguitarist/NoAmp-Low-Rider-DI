...
void PedalLookAndFeel::paintPedalBackground(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    const Blob blobs[] = {
        {0.18f, 0.15f, 110.f, 90.f, 0.60f, 0xFF19395Fu}, {0.82f, 0.22f, 80.f, 130.f, 0.55f, 0xFF0E203Eu},
        {0.50f, 0.68f, 100.f, 80.f, 0.50f, 0xFF12284Eu}, {0.22f, 0.82f, 70.f, 100.f, 0.45f, 0xFF0A1930u},
...
void PedalLookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int w, int h, float sliderPos,
    ...
    if (isTrim)
    {
        ...
        // Knob cap with radial gradient
        {
            juce::ColourGradient grad(juce::Colour(cKnobHighlight), cx - knobR * 0.3f, cy - knobR * 0.35f,
                                      juce::Colour(cKnobShadow), cx + knobR * 0.7f, cy + knobR * 0.7f, true);
        ...
        // Indicator line
        {
            ...
            g.drawLine(cx + inner * std::sin(valAngle), cy - inner * std::cos(valAngle),
                       cx + outer2 * std::sin(valAngle), cy - outer2 * std::cos(valAngle), 2.5f);
        }
    }
    else
    {
        ...
        {
            juce::ColourGradient grad(juce::Colour(cKnobHighlight), cx - knobR * 0.3f, cy - knobR * 0.35f,
                                      juce::Colour(cKnobShadow), cx + knobR * 0.6f, cy + knobR * 0.6f, true);
        ...
        // Indicator line
        {
            ...
            g.drawLine(cx + inner * std::sin(valAngle), cy - inner * std::cos(valAngle),
                       cx + outer2 * std::sin(valAngle), cy - outer2 * std::cos(valAngle), 3.0f);
        }
    }
}
                                            ...
                                            bool /*highlighted*/, bool down)
{
    ...
    if (button.getComponentID() == "bypass")
    {
        ...
        auto makeOctagon = [&](float radius) -> juce::Path
        {
            ...
            {
                const float a =
                    (float) i / 8.0f * juce::MathConstants<float>::twoPi + juce::MathConstants<float>::pi / 8.0f;
        ...
        // Octagonal nut — dark gunmetal gradient
        {
            juce::ColourGradient nutGrad(juce::Colour(0xFF8A8E94u), // light gunmetal top-left
                                         ...
                                         cx + nutR * 0.4f, cy + nutR * 0.5f, false);
        ...
        const float pressOff = down ? 1.5f : 0.0f;
        {
            ...
            g.drawEllipse(cx - domeR + 0.5f, cy - domeR + pressOff + 0.5f, domeR * 2.0f - 1.0f, domeR * 2.0f - 1.0f,
                          1.0f);
...
void PedalLookAndFeel::drawComboBox(juce::Graphics& g, int width, int height, bool isButtonDown, int /*buttonX*/,
                                    int /*buttonY*/, int /*buttonW*/, int /*buttonH*/, juce::ComboBox& /*box*/)
{
    ...
    // Background
    g.setColour(isButtonDown ? juce::Colour(cOSBtnActiveBdr).withAlpha(0.4f) : juce::Colour(cOSBtnActiveBg));
...
{
    return juce::Font(juce::FontOptions(juce::jmax(7.0f, (float) box.getHeight() * 0.38f), juce::Font::bold));
}
