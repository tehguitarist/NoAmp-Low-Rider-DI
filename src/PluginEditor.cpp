    ...
    : AudioProcessorEditor(&p), processorRef(p)
{
    ...
    osRealtimeAttach =
        std::make_unique<juce::ComboBoxParameterAttachment>(*apvts.getParameter(Proc::idOversampling), osRealtimeBox);
    ...
    scaleButton.onClick = [this]
    {
        ...
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(scaleButton),
                           ...
                           {
                               if (r >= 1 && r <= 9)
                                   setSize(juce::roundToInt(kBaseW * kScales[r - 1]),
                                           juce::roundToInt(kBaseH * kScales[r - 1]));
                               else if (r == 100)
    ...
    revisionAttach =
        std::make_unique<juce::ParameterAttachment>(*apvts.getParameter(Proc::idRevision), [this](float newValue)
                                                    { applyRevision((int) std::lround(newValue), false); });
    ...
    auto setupKnob =
        [this, &apvts](juce::Slider& s, const char* paramId, std::unique_ptr<juce::SliderParameterAttachment>& attach)
    {
        s.setSliderStyle(juce::Slider::RotaryHorizontalVerticalDrag);
        s.setTextBoxStyle(juce::Slider::NoTextBox, false, 0, 0);
        s.setRotaryParameters(kRotaryStart, kRotaryEnd, true);
...
void NoAmpLowRiderDIAudioProcessorEditor::applyRevision(int revision, bool forceLayout)
{
    ...
    juce::Component* const v2OnlyComponents[] = {&midSlider, &midShiftButton, &midShiftValue, &bassShiftButton,
                                                 &bassShiftValue};
...
void NoAmpLowRiderDIAudioProcessorEditor::layoutV1(juce::Rectangle<float> face, float sc)
{
    ...
    bypassLabel.setBounds(juce::Rectangle<float>(face.getX() + face.getWidth() * kFootswitchX - fsD * 0.6f,
                                                 ...
                                                 fsD * 1.2f, face.getHeight() * 0.035f)
                              .toNearestInt());
}
...
void NoAmpLowRiderDIAudioProcessorEditor::layoutV2(juce::Rectangle<float> face, float sc)
{
    ...
    auto placeShiftValue = [&face](DualValueLabel& l, float fx, float fy, float wFrac, float hFrac)
    {
        ...
        l.setTransform(
            juce::AffineTransform::rotation(juce::degreesToRadians(kShiftValueRotationDeg), centre.x, centre.y));
    };
