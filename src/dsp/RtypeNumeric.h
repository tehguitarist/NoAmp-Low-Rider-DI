...
namespace rtype
{
    ...
    inline double drivingPointResistance(int numPorts, int numNodes, const int* np, const int* nm,
                                         const double* portR, int upIndex) noexcept
    {
        ...
        auto gi = [&](int a, int b) -> double {
            return (a == kDatum || b == kDatum) ? 0.0 : Ginv[a * numNodes + b];
        };
    ...
    inline void scatteringMatrix(int numPorts, int numNodes, const int* np, const int* nm,
                                 const double* portR, double* outS) noexcept
    {
        double Gd[kMaxPorts] = {};
        double G[kMaxNodes * kMaxNodes] = {};
        for (int k = 0; k < numPorts; ++k)
        ...
        for (int i = 0; i < numPorts; ++i)
            ...
            {
                const double w = Gd[j]
                                 * (gi(np[i], np[j]) - gi(np[i], nm[j]) - gi(nm[i], np[j]) + gi(nm[i], nm[j]));
